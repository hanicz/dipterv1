"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
/**
 * Created by Hanicz on 2/19/2017.
 */
var core_1 = require("@angular/core");
var dropbox_service_1 = require("../services/dropbox.service");
var DropboxComponent = /** @class */ (function () {
    function DropboxComponent(dropboxService) {
        this.dropboxService = dropboxService;
    }
    DropboxComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.dropboxService.get_url().subscribe(function (json) {
            console.log(json);
            _this.url = JSON.parse(JSON.stringify(json)).url;
        }, function (error) { return console.error('Error: ' + error); });
    };
    DropboxComponent.prototype.finish_auth = function () {
        this.dropboxService.finish_auth(this.token).subscribe(function (json) {
            console.log(json);
        }, function (error) { return console.error('Error: ' + error); });
    };
    DropboxComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'dropbox',
            templateUrl: './dropbox.component.html',
            styleUrls: ['./dropbox.component.css']
        }),
        __metadata("design:paramtypes", [dropbox_service_1.DropboxService])
    ], DropboxComponent);
    return DropboxComponent;
}());
exports.DropboxComponent = DropboxComponent;
//# sourceMappingURL=dropbox.component.js.map